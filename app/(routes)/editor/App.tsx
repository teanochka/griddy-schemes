...
const Home = () => {
   ...
   */
  type CanvasObjectMap = Map<string, any>;
  const [canvasObjects, setCanvasObjects] = useState<CanvasObjectMap>(
    () => new Map()
);
   ...
   */
  const syncShapeInStorage = (object: fabric.Object | null) => {
    ...
    (shapeData as any).objectId = objectId;

    
    setCanvasObjects(prev => {
      const next = new Map(prev);
      next.set(objectId, shapeData);
      return next;
    });
  };
